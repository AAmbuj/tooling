...
def _normalize_execroot_namespace(path: str) -> str:
    """Normalize Bazel execroot absolute paths across sandbox namespaces.

    Bazel may expose the same execroot file via absolute paths that differ in
    the sandbox prefix (for example ``/.../sandbox/<id>/execroot/_main/...``).
    When ``/execroot/_main/`` is present, trim everything before it so strict
    prefix matching compares the stable execroot namespace only.
    """
...
def _node_source_key(
    ...
    (bare basename).  Matching is exact: the node's absolute path must have
    the workspace_offset as a prefix.  The remainder must match a canonical
    source key exactly.  There is no basename-based fallback: two same-
    basename diagrams in different packages require directory-preserving
    Bazel staging to distinguish them.
    ...
    workspace_offset = _normalize_execroot_namespace(
        os.fspath(workspace_offset).rstrip("/")
    )
    node_abs = _normalize_execroot_namespace(node_abs)
    if not node_abs.startswith(workspace_offset + "/"):
        return None
